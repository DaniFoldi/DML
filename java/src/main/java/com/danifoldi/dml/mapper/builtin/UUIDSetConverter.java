...
public class UUIDSetConverter implements DmlConverter {
    @Override
    public Object convertFrom(DmlCommentableValue dmlValue) {
        return dmlValue.asArray().value().stream().map(DmlValue::asString).map(v -> UUID.fromString(v.value())).collect(Collectors.toCollection(LinkedHashSet::new));
    }

    @Override
    public DmlCommentableValue convertTo(Object object, String comment) {
        return new DmlArray(new ArrayList<>(((Set<UUID>)object).stream().map(v -> (DmlValue)new DmlString(v.toString())).toList())).withComment(comment);
    }
